...
/*   Updated: 2016/01/04 13:15:14 by shamdani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			nb_link(char *name, t_env *e)
{
	...
	while (list_e)
	{
		if (ft_strcmp(list_e->link[1], name) == 0 || ft_strcmp(list_e->link[0], name) == 0)
			i++;
		list_e = list_e->next;
	}
	return (i);
...
static int		analyse(char *line, t_env *e, int i)
{
	...
	if (i == 0)
	{
		...
		return (e->end = 1);
	else if (line && line[0] == '#' && (ft_strcmp(line, "##start") != 0 || ft_strcmp(line, "##end") != 0))
		return (free_tab(&str, &str_p, 1));
